...
Owners can always adjust a balance. Managers and staff both start with the **Adjust points** permission switched on, so by default anyone on your team can fix a balance from the counter. The **Adjust/Deduct** tab only shows for people who hold that permission, so if you cannot see it, an owner has taken it off your account.

To take it off someone, open **Staff**, open the menu at the end of their row, and choose **Edit Permissions**. Turn on **Use custom permissions** and clear **Adjust points** under **Loyalty**. The tab stops appearing for them, and Qtap refuses the change if a request reaches it any other way.
...
## The member hears about the change

Both directions send the member a notice carrying the reason you typed and their new balance. A deduction arrives titled "Points adjusted" and an addition arrives as "+25 points", or whatever the amount was. Qtap sends it as a push notification and falls back to email if push is off. The dashboard only spells this out on the deduct side, where an orange note sits under the form and again on the confirmation screen, so write reasons a member will understand either way.
