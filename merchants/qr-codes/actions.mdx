...
A scan adds stamps or points only when Qtap knows who scanned. That happens when the customer scans from the Qtap app, when a staff member scans the customer's member code, or when someone new gives their phone number on the join page a camera scan opens. Until a person is attached to it, a scan counts toward the code's scan total and any limits you set, but it awards nothing.
